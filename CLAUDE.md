...
- `CQRS.Adapters.MassTransitMessageBus` — RabbitMQ-backed via MassTransit
...
- `CQRS.Application.MassTransitConsumers` — unwraps MassTransit context, passes DTO to Application handler; NO business logic here
...
- `CQRS.Projections.MassTransitConsumers` — same pattern as Application consumers
...
- **RabbitMQ 4.2** — message broker (MassTransit)
...
- **MassTransit** — message bus with RabbitMQ transport
